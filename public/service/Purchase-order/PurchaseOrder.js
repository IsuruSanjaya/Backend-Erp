...
const Phurchase_1 = __importDefault(require("../../models/Phurchase-order/Phurchase"));
 ...
 * @returns
 */
const __createPurchaseOrder = (__req, __res) => {
    try {
        const { date, transactionDate, orderNumber, referenceNumber, transactionType, purchaseOrder, shippingAddress, requestOrder, BillAddress, status, companyId, createAt } = __req.body;
        ...
        const purchase_Order = new Phurchase_1.default({
 ...
 * @returns
 */
const __updatePurchaseOrder = (__req, __res) => {
    try {
        const orderId = __req.params.id;
        return Phurchase_1.default.findById(orderId)
            .then((order) => {
            if (order) {
...
const __getAllOrders = (__req, __res) => {
    ...
    return Phurchase_1.default.find({ status: 1, companyId: companyId }, {})
 ...
 * @returns
 */
const __deletePurchaseOrder = (__req, __res) => {
    const __id = __req.params.id;
    return Phurchase_1.default.findByIdAndDelete(__id)
...
const __getById = (__req, __res) => {
    ...
    const companyId = __req.params.companyId;
    return Phurchase_1.default.findOne({ _id: __id, status: status, companyId: companyId }, {})
...
const __statusChange = (__req, __res) => {
    ...
    const status = __req.params.status;
    return Phurchase_1.default.findByIdAndUpdate(id, { status: status }, { new: true }, (err, doc) => {
...
exports.default = { __deletePurchaseOrder, __createPurchaseOrder, __updatePurchaseOrder, __getAllOrders, __statusChange, __getById, __getAllLov };
